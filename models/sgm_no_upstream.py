...
from layers.layers import AggregationLayer, Transformer, AggregationLayerV1


class SGM_no_upstream(BaseModel):
    def __init__(self, args):
        super(SGM_no_upstream, self).__init__(args)
        ...
        if len(self.user_neighbor_nums) >= 1 and np.prod(self.user_neighbor_nums[:1]) > 0:
            self.column_groups.update(
                {'ui': ['ui', 'ui_brand', 'ui_seller', 'ui_cate', 'ui_price'], 'ui_query': ['ui_query']})
        ...
        if len(self.item_neighbor_nums) >= 2 and np.prod(self.item_neighbor_nums[:2]) > 0:
            self.column_groups.update(
                {'iui': ['iui', 'iui_brand', 'iui_seller', 'iui_cate', 'iui_price'], 'iui_query': ['iui_query']})
    ...
    def create_model(self):
        ...
        agg_layer_u2i = AggregationLayerV1(self.dropout_rate)
        agg_layer_i2u = AggregationLayerV1(self.dropout_rate)
        ...
        target_uqi = group_embed_dict['item']
